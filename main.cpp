...
)
{
  ...
  fscanf(fp, "%d %d", &num_trees, &num_nodes);
 //
  ...
  for(int dt=1; dt <= num_trees; dt++) {
      ...
      for(int num_node = 0 ;num_node <num_nodes; num_node++) {
        ...
        fscanf(fp, "%d %d %d %d %lf %lf", &node_ID, &is_root, &right_child_ID, &left_child_ID, &div_time, &latent_value); // latent value not needed right now
        
    //    cout << node_ID << " " << is_root << " " << right_child_ID << " "<< " " << div_time << " " << latent_value << endl;
  ...
  //  cout << "Done initialization of the table" << endl;
    ...
    double likelihood = dft_log_prob_div(1, dt, st, 0, 0);
